...
Try {
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        # This may function better as parameter, or not.
        # set to 'private' to copy app source files to each users home drive and sets start menu shortcuts
        # 'public' copies source files to public home drive and sets desktop shortcut
        # both types set scheduled task to 'reprovision' source files from prog files directory.
        ...
        # also used to create the Application directory in one of the Program Files directories.
        $APPLICATION_NAME = '(($appname$))'
        # Location on local computer where source files will be copied ($APPLICATION_NAME = folder name)
        $SOURCE_FILE_DESTINATION = "(($sourcefolder$))"
        ...
        ## MSI installer execution with default parameters:
        ## $msifile = Get-Childitem -PAth "$dirFiles" -Filter "MSIFILENAME.msi" -file -Recurse -erroraction SilentlyContinue
        # $msifile = Get-Childitem -PAth "$dirFiles" -Filter "*.msi" -file -Recurse -erroraction SilentlyContinue
        # if ($msifile) {
        #     Write-Log -Message "Found $($msifile.fullname), running installation."
        #     Execute-MSI -Action 'Install' -Path "$($msifile.fullname)" -WindowStyle 'Hidden'
        # }

        ## Exe installer execution with supplied parameters:
        # $exefile = Get-Childitem -PAth "$dirFiles" -Filter "*.exe" -file -Recurse -erroraction SilentlyContinue
        # if ($exefile) {
        #     Write-Log -Message "Found $($exefile.fullname), running installation."
        #     Execute-Process -Path "$($exefile.fullname)" -Parameters "ADD SWITCHES HERE FOR SILENT INSTALL"
        # }
        ...
        ## GET USERINSTALL.PS1 script - used to provision both public/private installations and set scheduled task
