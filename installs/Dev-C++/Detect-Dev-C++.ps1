# detects any dev c++ scheduled task - either user or public
$SCHED_TASK_TITLE = "Dev-C++"
...
# check for devcppportable.exe
if ($installtype -eq 'public') {
    $check_for_public_exe = Get-Childitem -Path "C:\Users\Public\Dev-C++\" -filter "devcppportable.exe" -file -erroraction silentlycontinue
}
$check_for_exe = Get-Childitem -Path "C:\Program Files (x86)\Dev-C++" -filter "devcppportable.exe" -file -erroraction silentlycontinue
...
if (($test_for_task) -and ($check_for_exe)) {
    ...
    # has to be private installation at this point - and conditions have already been filled - report success
        ...
        Write-Host "Not public install - scheduled task and prog files x86 exe found. Installed."
        Exit 0
    }
}
else {
