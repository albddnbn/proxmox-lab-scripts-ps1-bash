...
Try {
    ...
    [string]$appScriptDate = '09/22/2023' # updated: 12-24-2023
    [string]$appScriptAuthor = 'Jason Bergner' # edited: DTCC\abuddenb
    ##*===============================================
    ## Variables: Install Titles (Only set here to override defaults set by the toolkit)
    [string]$installName = ''
    [string]$installTitle = 'Visual Studio Code'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        Show-InstallationProgress -StatusMessage "Removing Any Existing Versions of $installTitle. This may take some time, please wait..."
        ...
        #         Execute-Process -Path "$ExePath32" -Parameters "/ALLUSERS /VERYSILENT /NORESTART /SUPPRESSMSGBOXES /SP- /MERGETASKS=!runcode /LOG=""$configToolkitLogDir\VSCode32-Install.log""" -WindowStyle Hidden
        #         Start-Sleep -Seconds 5
        ...
        # <# 64-bit section --------------------------------------------------------------------------------------------- #>
        # Else { 
        Write-Log -Message "Script is build for 64-bit OS Architecture $installTitle installations." -Severity 1 -Source $deployAppScriptFriendlyName

        ## Install Microsoft Visual Studio Code (64-bit Systems)
        $ExePath64 = Get-ChildItem -Path "$dirFiles" -Filter "VSCodeSetup-x64*.exe" -File -ErrorAction SilentlyContinue
        If ($ExePath64) {
            if ($ExePath64.count -gt 1) {
                $ExePath64 = $ExePath64 | Sort -Property VersionInfo.FileVersion -Descending | Select -First 1
                Write-Log -Message "Found multiple .exe 's that match the filter 'VSCodeSetup-x64*.exe' in $dirFiles. Chose latest version: $($exePath64.fullname)."
            }
            ...
            Execute-Process -Path "$($ExePath64.Fullname)" -Parameters "/ALLUSERS /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /MERGETASKS=!runcode /SP- /LOG=""$configToolkitLogDir\VSCode64-Install.log""" -WindowStyle Hidden
            Start-Sleep -Seconds 5
            ...
            If ($JSON.Exists) {
                ...
                ForEach ($Profile in $UserProfiles) {
                    ...
                    New-Item "$Profile\AppData\Roaming\Code\User\" -ItemType Directory -Force | Out-null
                    Copy-Item $JSON -Destination "$Profile\AppData\Roaming\Code\User\" -ErrorAction SilentlyContinue
                }
            }
        }
        else {
            Write-Log -Message "FILE NOT FOUND: Couldn't find the 64-bit installer file in $dirFiles" -Severity 3
        ...
        # stop code.exe if its running
        # get-process | where-object { $_.Name -like "*code*" } | Stop-Process -Force
        Write-Log -Message "Terminating all 'code.exe' processes, so that extensions can be installed." -Severity 2
        Get-Process -Name 'code' -Erroraction SilentlyContinue | Stop-Process -Force

        # get the code binary from program files
        # $code = Get-ChildItem -Path "C:\Program Files\Microsoft VS Code\bin\" | where-object { $_.Name -like "*code.cmd*" }
        $code_cmd = Get-ChildItem -Path "$env:ProgramFiles\Microsoft VS Code\bin" -Filter "*code.cmd" -File -Recurse -ErrorAction SilentlyContinue

        # "yandeu.five-server" (Five Server extension) is an updated/improved version of 'Live Server' extension:  "ritwickdey.liveserver"
        # "ms-python.autopep8" formatted / PEP8 compliance/checking for Python
        # "mathcale.theme-dracula-refined" is a Dracula theme for VS Code
        $extensions = "msjsdiag.vscode-react-native", "esbenp.prettier-vscode", "ms-python.autopep8", "mathcale.theme-dracula-refined", "yandeu.five-server"
        ForEach ($ext in $extensions) {
            Write-Log -Message "Installing $ext extension"
            & "$($code_cmd.FullName)" --install-extension $ext --force
        }
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        $AppList = Get-InstalledApplication -Name 'Visual Studio Code'        
        ForEach ($App in $AppList) {
            If ($App.UninstallString) {
                $UninstPath = $($App.UninstallString).Replace('"', '')       
                If (Test-Path -Path $UninstPath) {
