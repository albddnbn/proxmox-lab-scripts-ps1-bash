...
Try {
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        # Remove-Applications -Name "*AppName*" -Wildcard -FilterApplication ('Publisher', 'App Publisher', 'Contains')

        ## Removing existing versions of .exe
        # $AppList = Get-InstalledApplication -Name "*PyCharm*" -Wildcard
        # ForEach ($App in $AppList) {
        #     If ($App.Publisher -like "*JetBrains*") {
        ...
        $PathToExe = Get-ChildItem -Path "$dirFiles" -Include "setup.exe" -File -ErrorAction SilentlyContinue
